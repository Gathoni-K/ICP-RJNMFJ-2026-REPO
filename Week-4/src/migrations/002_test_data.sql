...
-- LEFT JOIN fetches all students even if they have no grades, students with no grades will show up with NULL for average and 0 for COUNT
-- GROUP BY s.id - groups all the grade rows per student so the COUNT and AVG functions can calculate per student
GROUP BY s.id
ORDER BY average_score DESC NULLS LAST;
-- NULLS LAST - pushes students with no grades to the bottom
